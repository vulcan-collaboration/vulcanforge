...
from vulcanforge.project.tasks import (
    unindex_project,
    reindex_project,
    unpublish_artifacts
)
...
class ProjectExtension(MapperExtension):
    ...
    def after_update(self, instance, state, sess):
        changed = get_dict_diff_have_keys_changed(
...
class Project(SOLRIndexed):
    ...
    neighborhood = VFRelationProperty(Neighborhood)
    ...
    @classmethod
    def check_is_my_kind(cls, instance):
        property_name = cls.__mongometa__.polymorphic_on
        identity = cls.__mongometa__.polymorphic_identity
        try:
            return getattr(instance, property_name) == identity
        except:
            return False
    ...
    def app_configs(self):
        return AppConfig.query.find({
            'project_id': self._id}).sort('options.ordinal').all()
    ...
    @property
    def private(self):
        public_roles = ('*anonymous', '*authenticated')
        role_names = set([x.name for x in self.get_expanded_read_roles()])
        return role_names.isdisjoint(public_roles)
    ...
    def install_app(self, ep_name, mount_point=None, mount_label=None,
        ...
        if acl is None and self.neighborhood.project_template:
            ...
            acl = project_template.get('tool_acl', {}).get(ep_name.lower())
    ...
    def user_invited(self, user):
        ...
        if mq.get(project_id=self._id, user_id=user._id):
            return True
        # case insensitive email matching
        user_addresses = [x.lower() for x in user.email_addresses]
        q = {'project_id': self._id, "email": {"$ne": None}, "user_id": None}
        invites = [x for x in mq.find(q) if x.email.lower() in user_addresses]
        if invites:
    ...
    def user_requested(self, user):
        try:
            from vulcanforge.project.model import MembershipRequest
        except ImportError:
            return False
        return bool(
    ...
    def user_requested_leave(self, user):
        try:
            from vulcanforge.project.model import MembershipCancelRequest
        except ImportError:
            return False
        return bool(
    ...
    def user_removal_requested(self, user):
        try:
            from vulcanforge.project.model import MembershipRemovalRequest
        except ImportError:
            return False
        return bool(
    ...
    def user_join_project(self, user, role=None, notify=False,
        ...
        self._add_user_to_role(user, role)
        try:
            from vulcanforge.project.model import (
                MembershipRequest, MembershipInvitation)
        except ImportError:
            LOG.exception("Could not import admin app")
            MembershipRequest, MembershipInvitation = None, None
        if MembershipRequest is not None:
            q = {'user_id': user._id, 'project_id': self._id}
            MembershipRequest.query.remove(q)
            MembershipInvitation.query.remove(q)
    ...
    def user_leave_project(self, user, clean_roles=True, notify=False,
        ...
        for project in user.my_projects():
            if project._id != self._id and project.is_real():
                break
        else:
            user.has_project = False
    ...
    def add_user(self, user, role_names):
        ...
        for role_name in role_names:
            role = ProjectRole.by_name(role_name, self)
            self._add_user_to_role(user, role, pr)

    def _add_user_to_role(self, user, role=None, project_role=None):
        if role is None:
            role = self.default_role
        if project_role is None:
            project_role = self.project_role(user)

        if role._id not in project_role.roles:
            project_role.roles.append(role._id)
    ...
    def delete_project(self, user=None):
        ...
        unpublish_artifacts.post(project_id=self._id)
...
class AppConfig(MappedClass):
    ...
    def lookup_label_counts(self):
        app = self.load()
        labels = {}
        for artifact_spec in app.artifacts.values():
            if not artifact_spec.get("model"):
                continue
            artifact_cls = artifact_spec["model"]
...
class ProjectRole(BaseMappedClass):
    ...
    def parent_roles(self):
        ...
        return self.query.find({'roles': { '$in': [self._id]}}).all()
