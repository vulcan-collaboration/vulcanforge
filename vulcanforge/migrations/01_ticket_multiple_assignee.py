...
from vulcanforge.tools.tickets.model import Ticket, Globals, Bin
from vulcanforge.tools.tickets.tasks import add_tickets


class TicketMultipleAssignees(BaseMigration):
    def run(self):
        db, coll = pymongo_db_collection(Ticket)
        ref_ids = []
        ac_ids = set()
        ticket_count = 0
        for ticket_doc in coll.find({"assigned_to_id": {"$exists": 1}}):
            ticket_doc['assigned_to_ids'] = [ticket_doc.pop('assigned_to_id')]
            coll.save(ticket_doc)
