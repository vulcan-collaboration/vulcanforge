...
// From: https://github.com/23/resumable.js
    ...
    var Resumable = function(opts){
        ...
        $.defaults = {
            ...
            fileTypeErrorCallback: function(file, errorCount) {
                alert(file.fileName||file.name +' \'s type is not allowed, please upload files of type ' + $.getOpt('fileType') + '.');
            }
        };
        $.opts = opts||{};
        $.getOpt = function(o) {
        ...
        var $h = {
            ...
            getTarget:function(file, params){
                var target = $.getOpt('target');
                if(typeof target === 'function') {
                    return target(file, params);
                }
                if(target.indexOf('?') < 0) {
                    target += '?';
                } else {
                        ...
                        reader.readEntries(
                            ...
                            function(entries){
                                if (entries.length>0){
                                    ...
                                    loadFiles(newEntries, event, queue, dirPath);
        ...
        function ResumableChunk(resumableObj, fileObj, offset, callback){
            ...
            $.test = function(){
                ...
                $.xhr.open($.getOpt('testMethod'), $h.getTarget($.fileObj, params));
            ...
            $.send = function(){
                ...
                if ($.getOpt('method') === 'octet') {
                    ...
                    target = $h.getTarget($.fileObj, params);
                } else {
                    // Add data from the query options
                    data = new FormData();
                    $h.each(query, function(k,v){
        ...
        $.assignBrowse = function(domNodes, isDirectory){
            ...
            $h.each(domNodes, function(domNode) {
                ...
                if(domNode.tagName==='INPUT' && domNode.type==='file'){
                    ...
                    // adding file extensions when specified
                    if (acceptList != ''){
