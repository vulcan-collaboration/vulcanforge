...
class AlluraUser(S.FancySchemaItem):
    ...
    def _validate(self, value, **kw):
        ...
        vf_username = self.mapping.get(value, value)
        result = User.by_username(vf_username)
        if result is None:
            self.warnings.append('User %s not found' % value)
            if callable(self.default_username):
                vf_username = self.default_username(value)
            else:
                vf_username = self.default_username
            self.warnings.append('... setting username to %r' % vf_username)
            result = User.by_username(vf_username)
            self.mapping[value] = vf_username
        return result
