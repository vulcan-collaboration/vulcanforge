...
from vulcanforge.auth.model import UserRegistrationToken
...
You may accept this invitation by visiting {} {}
"""
...
class MembershipRemovalRequest(AbstractRequestArtifact):
    ...
    initiator = RelationProperty('User', via="initiator_id")
...
class MembershipInvitation(Artifact):
    ...
    creator = RelationProperty('User', via="creator_id")
    text = FieldProperty(str)

    def index(self, **kw):
        return dict(
            title_s=self.invitation_subject
        )
    ...
    @classmethod
    def from_email(cls, email, project=None, text=''):
        if project is None:
            project = c.project
